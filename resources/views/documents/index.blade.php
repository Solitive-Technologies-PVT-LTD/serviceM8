    ...
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex items-center space-x-3">
                ...
                <!-- Upload files -->
                <form action="{{ route('documents.file.store') }}" method="POST" enctype="multipart/form-data" class="inline-block">
                    @csrf
        ...
        <div class="mb-4">
            <nav class="flex items-center space-x-2 text-sm">
                <a href="{{ route('documents.index') }}" class="text-toms-green hover:underline">Home</a>
                @if($breadcrumb)
                    @foreach($breadcrumb as $folder)
                        <span class="text-gray-400">/</span>
                        <a href="{{ route('documents.index', $folder->id) }}" class="text-toms-green hover:underline">{{ $folder->name }}</a>
                    @endforeach
                @endif
            </nav>
        </div>
            ...
            <div id="gridView" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @forelse($folders as $folder)
                    <div class="p-4 bg-white shadow hover:shadow-lg rounded-lg flex flex-col items-center transition-transform transform hover:-translate-y-1">
                        <a href="{{ route('documents.index', $folder->id) }}" class="flex flex-col items-center text-center">
                        ...
                        <form action="{{ route('documents.folder.delete', $folder->id) }}" method="POST" class="mt-2">
                            @csrf
                            @method('DELETE')
                ...
                @forelse($files as $file)
                    <div class="p-4 bg-white shadow hover:shadow-lg rounded-lg flex flex-col items-center transition-transform transform hover:-translate-y-1">
                        ...
                        <div class="flex space-x-2 mt-2">
                            <a href="{{ route('documents.file.download', $file->id) }}" class="text-green-600 hover:bg-green-50 px-2 py-1 rounded transition text-sm">Download</a>
                        ...
                        @foreach($folders as $folder)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4 flex items-center space-x-3">
                                    ...
                                    <a href="{{ route('documents.index', $folder->id) }}" class="text-gray-900 font-medium">{{ $folder->name }}</a>
                                </td>
                                <td class="px-6 py-4">Folder</td>
                                <td class="px-6 py-4">
                                    <form action="{{ route('documents.folder.delete', $folder->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:bg-red-50 px-2 py-1 rounded transition text-sm">Delete</button>
                                    </form>
