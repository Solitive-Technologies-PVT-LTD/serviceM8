...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <form method="post" action="{{route('role-save')}}" name="add-setting-form" id="add-setting-form" enctype="multipart/form-data"> 
                @csrf
                        ...
                        <div class="row gy-4 mt-3">
                            <div class="col-lg-12 ">
                                <div>
                                    ...
                                    <input type="text" class="form-control @error('role_name') is-invalid @enderror" name="role_name" id="role_name" value="{{ old('role_name') }}" placeholder="Enter Role Name">
                                    @error('role_name')
                                        ...
                                        <div id="permissions">
                                            <ul>
                                                @foreach($permissions as $permission)
                                                    ...
                                                    ?>
                                                     <li @if( old('selected_permissions') && in_array($permission->id, $selected) )) data-jstree='{"selected":true}' @endif id="{{ $permission->id }}">{{ $permission->name }}
                                                            <ul>
                                                                @foreach($permission->children as $sub_permission)
                                                                <li @if(old('selected_permissions') && in_array($sub_permission->id, $selected)) data-jstree='{"selected":true}' @endif id="{{ $sub_permission->id }}">{{ $sub_permission->name }} </li> 
                                                                @endforeach
                                                            </ul>
                                                        </li>
                                                    @else
                                                     <li @if(old('selected_permissions') && in_array($permission->id,$selected))) data-jstree='{"selected":true}' @endif id="{{ $permission->id }}">{{ $permission->name }}</li> 
                                                    @endif
                                                @endforeach
                                            </ul>
                                        </div>
